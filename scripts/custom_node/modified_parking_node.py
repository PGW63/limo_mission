...
class ParkingNode:
    def __init__(self):
        ...
        self.forward_time = rospy.get_param('~forward_time', 2.0)              # 주차 구역 지나 전진 시간 (초)
        self.reverse_right_time = rospy.get_param('~reverse_right_time', 3.25)  # 후진 우회전 시간 (초)
        self.reverse_left_time = rospy.get_param('~reverse_left_time', 3.25)    # 후진 좌회전 시간 (초)
        self.final_forward_time = rospy.get_param('~final_forward_time', 0.7)  # 마지막 전진 직진 시간 (초)
    ...
    def control_loop(self, event):
        ...
        # IDLE 상태가 아닐 때만 cmd_vel 발행 (IDLE은 lane detection이 제어)
        if self.state != ParkingState.IDLE:
